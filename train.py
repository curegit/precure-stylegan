...
parser.add_argument("-k", "--current", action="store_true", help="save completed models in the current directory")
...
parser.add_argument("-f", "--force", action="store_true", help="allow overwriting existing files")
...
parser.add_argument("-g", "--generator", metavar="FILE", help="HDF5 file of a serialized trained generator to load and retrain")
parser.add_argument("-G", "--averaged-generator", metavar="FILE", dest="averaged", help="HDF5 file of a serialized averaged generator to load and continue")
parser.add_argument("-d", "--discriminator", metavar="FILE", help="HDF5 file of a serialized trained discriminator to load and retrain")
...
parser.add_argument("-c", "--channels", metavar="CH", type=natural, nargs=2, default=(512, 16), help="the numbers of channels at the initial stage and final stage")
parser.add_argument("-z", "--z-size", dest="size", type=natural, default=512, help="latent vector (feature vector) size")
parser.add_argument("-m", "--mlp-depth", dest="depth", type=natural, default=8, help="MLP depth of the mapping network")
parser.add_argument("-n", "--number", type=uint, default=10, help="the number of middle images to generate")
parser.add_argument("-b", "--batch", type=natural, default=16, help="batch size, affects memory usage")
parser.add_argument("-e", "--epoch", type=natural, default=1, help="training duration in epoch")
parser.add_argument("-a", "--alpha", type=rate, default=0.0, help="the blend alpha")
...
parser.add_argument("-D", "--decay", type=rate, default=0.999, help="coefficient of exponential moving average for generator weights")
...
parser.add_argument("-u", "--print-interval", metavar="ITER", dest="print", type=uint, nargs=2, default=(5, 500), help="specify print intervals of progress bar and statistics")
parser.add_argument("-l", "--write-interval", metavar="ITER", dest="write", type=uint, nargs=4, default=(1000, 3000, 500, 500), help="specify intervals to save middle images, snapshots, statistics and charts")
parser.add_argument("-v", "--device", "--gpu", metavar="ID", dest="device", type=device, default=-1, help="use the specified GPU or CPU device")
